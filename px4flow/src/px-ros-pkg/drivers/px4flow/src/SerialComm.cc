...
SerialComm::open(const std::string& portStr, int baudrate)
{
    ...
    m_optFlowLocalNEDPub = nh.advertise<geometry_msgs::PoseStamped>("px4flowLocalNEDraw", 20);
...
SerialComm::readCallback(const boost::system::error_code& error, size_t bytesTransferred)
{
    ...
    for (size_t i = 0; i < bytesTransferred; i++) {
        ...
        if (msgReceived)
        {
            ...
            {
            	ROS_INFO("OPTICAL FLOW RAD MESSAGE");
            	break;
            }

            case MAVLINK_MSG_ID_RAW_IMU:
            {
            	ROS_INFO("RAW_IMU Message");
            	break;
            }
            ...
            case MAVLINK_MSG_ID_LOCAL_POSITION_NED:
            {
                ...
                geometry_msgs::PoseStamped Px4FlowLocalNEDraw;


                mavlink_msg_local_position_ned_decode(&message, &local_NED);
                Px4FlowLocalNEDraw.header.seq = local_ned_counter;
                Px4FlowLocalNEDraw.header.stamp = ros::Time::now();
                Px4FlowLocalNEDraw.header.frame_id = m_frameId;

                Px4FlowLocalNEDraw.pose.position.x = local_NED.x;
                Px4FlowLocalNEDraw.pose.position.y = local_NED.y;
